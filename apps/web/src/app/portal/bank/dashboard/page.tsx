...
export default function BankDashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiClient.get<PortfolioItem[]>("/portfolios"),
    ])
      .then(([kpiData, portfolioData]) => {
        setKpis(kpiData);
  ...
  const totalAssigned = portfolios.reduce((s, p) => s + Number(p.totalAmount), 0);
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">
                  {formatCurrency(kpis?.totalPaymentsAmount || 0)}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">{activeCases.toLocaleString("es-DO")}</p>
              </div>
            ...
            <div className="mt-2 text-xs text-slate-500">
              <span className="font-medium text-amber-700">{kpis?.totalAgreements || 0}</span> con acuerdo
              en vigor
            </div>
          </CardContent>
        </Card>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyPerformance}>
                  ...
                  <YAxis tick={{ fontSize: 10 }} stroke="#94a3b8" tickFormatter={(v) => `${(v / 1000000).toFixed(0)}M`} />
                  ...
                  <Legend />
                  <Area type="monotone" dataKey="assigned" stroke="#6366f1" strokeWidth={2} fill="url(#colorAssigned)" name="Asignado" />
                  <Area type="monotone" dataKey="recovered" stroke="#10b981" strokeWidth={2} fill="url(#colorRecovered)" name="Recuperado" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={conversionTrend}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                  <XAxis dataKey="month" tick={{ fontSize: 12 }} stroke="#94a3b8" />
                  <YAxis domain={[0, 25]} tick={{ fontSize: 10 }} stroke="#94a3b8" tickFormatter={(v) => `${v}%`} />
                  <Tooltip formatter={(value) => `${value}%`} contentStyle={{ backgroundColor: "#1e293b", border: "none", borderRadius: "8px", color: "#fff" }} />
                  <Line type="monotone" dataKey="rate" stroke="#8b5cf6" strokeWidth={3} dot={{ r: 4, fill: "#8b5cf6", strokeWidth: 2, stroke: "#fff" }} activeDot={{ r: 6 }} name="Conversión" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="space-y-3">
              {alerts.map((alert) => (
                <div key={alert.id} className={`flex items-start gap-3 p-3 rounded-lg border ${alert.bg} ${alert.border}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${alert.bg}`}>
              ...
              <TableBody>
                {portfolios.map((pf) => (
                  <TableRow key={pf.id} className="hover:bg-slate-50">
                    <TableCell>
                      <div>
                        <p className="font-medium text-slate-900">{pf.name}</p>
                        <p className="text-xs text-slate-400">{pf.id.toUpperCase()}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className={statusColor[pf.status] || "bg-slate-100 text-slate-800 border-slate-200"}>
                        {statusLabel[pf.status] || pf.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="font-medium text-slate-900">
                      {formatCurrency(Number(pf.totalAmount))}
                    </TableCell>
                    <TableCell>
                      <span className="text-xs text-slate-600">{pf._count?.cases ?? 0} casos</span>
                    </TableCell>
                  </TableRow>
                ))}
                {portfolios.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center text-slate-500 py-8">No hay carteras registradas.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
