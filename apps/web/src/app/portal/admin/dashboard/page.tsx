...
import { demoCommunications, demoAuditLog, formatCurrency, formatDate } from "@/lib/seed-data";
...
interface CaseItem {
  ...
  dataPassport?: { confidence: number };
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    setMounted(true);
    Promise.all([
      apiClient.get<KpiData>("/reports/kpis"),
      apiClient.get<CaseItem[]>("/cases"),
    ])
      .then(([kpiData, casesData]) => {
        setKpis(kpiData);
        setCases(casesData);
        setLoading(false);
      })
  ...
  // Prisma Decimal serializes as a string (e.g. totalBalance: "250000"), so
  // coerce with Number() — otherwise the reduce concatenates strings and
  // produces "025000089000125000" instead of the numeric sum.
  const totalBalance = cases.reduce((sum, c) => sum + Number(c.totalBalance || 0), 0);
  const activeCases = kpis?.casesByStatus?.ACTIVE || 0;
  const casesWithAgreement = kpis?.totalAgreements || 0;
        ...
        <Card className="border-l-4 border-l-emerald-500 bg-gradient-to-br from-emerald-50/50 to-white">
          <CardContent className="p-4">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="text-xs text-slate-500">{activeCases} casos</span>
              <span className="text-xs text-slate-300">·</span>
              <span className="text-xs text-emerald-600 font-medium">{casesWithAgreement} con acuerdo</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">{formatCurrency(kpis?.totalPaymentsAmount || 0)}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-700 mt-1">{blockedCount} protecciones</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-slate-900">{(kpis?.totalCases || 0) > 0 ? ((casesWithAgreement / (kpis?.totalCases || 1)) * 100).toFixed(1) : "0"}%</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-slate-900">${totalComms > 0 ? Math.round(totalBalance / totalComms) : 0}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-slate-900">{kpis?.casesByStatus?.BLOCKED || 0} activas</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <Table>
            ...
            <TableBody>
              {cases.slice(0, 8).map((c) => {
                ...
                const contactScore = c.scoreContactability ?? 0;
                const riskScore = c.scoreRisk ?? 0;
                ...
                return (
                  <TableRow key={c.id} className="hover:bg-slate-50">
                    ...
                    <TableCell className="text-sm text-slate-600">{productType}</TableCell>
                    <TableCell className="font-medium text-slate-900">{formatCurrency(c.totalBalance)}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className={statusColor[status] || "bg-slate-100 text-slate-800 border-slate-200"}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-slate-500">{contactScore}%</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-2 bg-slate-200 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${riskScore > 70 ? "bg-red-500" : riskScore > 40 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${riskScore}%` }} />
                        </div>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-slate-600">{c.dataPassport?.confidence ?? 85}%</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1.5 text-xs">
                        {c.nextAction === "call" && <Phone className="w-3 h-3 text-emerald-600" />}
                        {c.nextAction === "portal" && <Mail className="w-3 h-3 text-blue-600" />}
                        {!c.nextAction && <AlertTriangle className="w-3 h-3 text-red-500" />}
                        <span className={!c.nextAction ? "text-red-600 font-medium" : "text-slate-600"}>
                          {c.nextAction === "call" ? "Llamar" : c.nextAction === "portal" ? "Portal" : "Pausado"}
                        </span>
                      </div>
                    </TableCell>
                  </TableRow>
        ...
        <CardContent>
          <div className="space-y-3">
            {demoCommunications.filter((c) => c.status === "blocked").map((comm) => {
              const caseData = cases.find((c) => c.id === comm.caseId);
              return (
                <div key={comm.id} className="flex items-start gap-3 p-3 rounded-lg bg-emerald-50 border border-emerald-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-1">{caseData ? `${caseData.debtor.firstName} ${caseData.debtor.lastName}` : "Desconocido"} · {formatDate(comm.timestamp)}</p>
                  </div>
                </div>
              );
            })}
