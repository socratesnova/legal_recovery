...
const portfolio_rules_service_1 = require("./portfolio-rules.service");
...
describe("PortfolioRulesService", () => {
    ...
    beforeEach(async () => {
        ...
        service = moduleRef.get(portfolio_rules_service_1.PortfolioRulesService);
    });
    ...
    describe("getForPortfolio", () => {
        ...
        it("throws 404 when the portfolio does not exist", async () => {
            ...
            await expect(service.getForPortfolio("nope", gestor)).rejects.toBeInstanceOf(common_1.NotFoundException);
        ...
        it("throws 403 when the portfolio belongs to another institution", async () => {
            ...
            await expect(service.getForPortfolio("port1", gestor)).rejects.toBeInstanceOf(common_1.ForbiddenException);
        });
        ...
        it("treats a soft-deleted portfolio as not found", async () => {
            ...
            await expect(service.getForPortfolio("port1", gestor)).rejects.toBeInstanceOf(common_1.NotFoundException);
        });
    });
    describe("upsertForPortfolio", () => {
        it("creates the rule when none exists", async () => {
            ...
            const result = await service.upsertForPortfolio("port1", { discountMax: 15, minInstallments: 3 }, gestor);
            expect(result).toEqual(created);
            expect(prisma.portfolioRule.upsert).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("throws 403 when the portfolio belongs to another institution", async () => {
            ...
            await expect(service.upsertForPortfolio("port1", { discountMax: 10 }, gestor)).rejects.toBeInstanceOf(common_1.ForbiddenException);
        ...
        it("allows super_admin to upsert any tenant's rule", async () => {
            ...
            prisma.portfolioRule.upsert.mockResolvedValue({ portfolioId: "port1" });
            const result = await service.upsertForPortfolio("port1", { discountMax: 10 }, superAdmin);
            expect(result).toEqual({ portfolioId: "port1" });
        });
    });
});
//# sourceMappingURL=portfolio-rules.service.spec.js.map