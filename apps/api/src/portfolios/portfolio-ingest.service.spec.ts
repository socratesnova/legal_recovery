...
describe("PortfolioIngestService", () => {
  ...
  beforeEach(async () => {
    ...
    service = moduleRef.get(PortfolioIngestService);
  });
  ...
  describe("normalizeRow", () => {
    ...
    it("returns null when required fields are missing", () => {
      expect(
        service.normalizeRow({ cedula: "001", nombres: "Juan", saldo: "" }),
      ).toBeNull();
      expect(
        service.normalizeRow({ cedula: "", nombres: "Juan", saldo: "100" }),
      ).toBeNull();
      expect(
        service.normalizeRow({ cedula: "001", nombres: "", saldo: "100" }),
      ).toBeNull();
    });
    it("returns null when balance is non-numeric", () => {
      expect(
        service.normalizeRow({
          cedula: "001",
          nombres: "Juan",
          apellidos: "P",
          saldo: "N/A",
  ...
  describe("parse (CSV)", () => {
    it("parses a semicolon-delimited CSV buffer", async () => {
      const csv =
        "cedula;nombres;apellidos;saldo\n001;Juan;Perez;1500.50\n002;Maria;Lopez;2300.75";
      const rows = await service.parse(Buffer.from(csv), "port.csv");
      expect(rows).toHaveLength(2);
      expect(rows[0]).toEqual(
        expect.objectContaining({ idNumber: "001", totalBalance: 1500.5 }),
      );
      expect(rows[1]).toEqual(
        expect.objectContaining({ idNumber: "002", totalBalance: 2300.75 }),
      );
    });
    ...
    it("returns an empty array for a header-only buffer", async () => {
      const rows = await service.parse(
        Buffer.from("cedula;nombres;apellidos;saldo"),
        "port.csv",
      );
      expect(rows).toEqual([]);
    });
    ...
    it("throws BadRequest when the CSV has an unterminated quote", async () => {
      await expect(
        service.parse(Buffer.from('a,b,"c'), "port.csv"),
  ...
  describe("ingest", () => {
    ...
    beforeEach(() => {
      ...
      prisma.debtor.create.mockImplementation(
        ({ data }: { data: { idNumber: string } }) =>
          Promise.resolve({ id: `d-${data.idNumber}` }),
      );
    ...
    it("rejects ingest for another tenant's portfolio", async () => {
      ...
      await expect(service.ingest(file, "p1", admin)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
    ...
    it("reuses the debtor when create hits a unique-constraint race (P2002)", async () => {
      const p2002 = new Prisma.PrismaClientKnownRequestError(
        "Unique constraint failed on id_number",
        { code: "P2002", clientVersion: "6.19.3" },
      );
      // Initial lookup finds nothing; create loses the race (P2002); the
      // re-fetch finds the row created by the concurrent ingest.
      prisma.debtor.findFirst
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce({ id: "d-001" });
