...
import {
  ...
  ReconcileResult,
} from "./delivery-reconciler.service";
...
function makeReq(): Request {
  return {
    protocol: "https",
    get: (h: string) =>
      h.toLowerCase() === "host" ? "api.example.com" : undefined,
...
function makeReconciler(
  ...
  } as unknown as DeliveryReconcilerService;
}

const TWILIO_TOKEN = "test-twilio-token";
const TWILIO_URL =
  "https://api.example.com/api/v1/communications/webhooks/twilio";

function twilioSig(body: Record<string, string>): string {
  const data =
    TWILIO_URL +
    Object.keys(body)
      .sort()
...
describe("DeliveryWebhookController", () => {
  describe("POST /twilio", () => {
    it("returns no-op when MessageSid/MessageStatus are missing", async () => {
      ...
      const ctrl = new DeliveryWebhookController(reconciler, makeConfig({}));
    ...
    it("reconciles in dev mode (no TWILIO_AUTH_TOKEN)", async () => {
      const reconciler = makeReconciler({
        communicationId: "c1",
        previousStatus: CommStatus.SENT,
        status: CommStatus.DELIVERED,
        updated: true,
      });
      const ctrl = new DeliveryWebhookController(reconciler, makeConfig({}));
      const res = await ctrl.twilio(
        makeReq(),
    ...
    it("passes with a valid signature and reconciles", async () => {
      const reconciler = makeReconciler({
        communicationId: "c1",
        previousStatus: CommStatus.SENT,
        status: CommStatus.READ,
        updated: true,
      });
      const ctrl = new DeliveryWebhookController(
        reconciler,
        makeConfig({ TWILIO_AUTH_TOKEN: TWILIO_TOKEN }),
      );
  ...
  describe("POST /delivery", () => {
    ...
    it("reconciles in dev mode (no shared secret)", async () => {
      ...
      const ctrl = new DeliveryWebhookController(reconciler, makeConfig({}));
      const res = await ctrl.delivery(
        { providerMessageId: "msg-1", status: "delivered" },
        undefined,
      );
      expect(res).toEqual({ ok: true, reconciled: true });
      expect(reconciler.reconcile).toHaveBeenCalledWith({
    ...
    it("accepts the messageId alias and a custom provider", async () => {
      ...
      const ctrl = new DeliveryWebhookController(reconciler, makeConfig({}));
      await ctrl.delivery(
        { messageId: "msg-1", status: "delivered", provider: "ses" },
        undefined,
      );
      expect(reconciler.reconcile).toHaveBeenCalledWith({
        providerMessageId: "msg-1",
    ...
    it("rejects a wrong shared secret with ForbiddenException", async () => {
      const ctrl = new DeliveryWebhookController(
        makeReconciler(null),
        makeConfig({ DELIVERY_WEBHOOK_SECRET: "shh" }),
      );
      await expect(
        ctrl.delivery(
          { providerMessageId: "msg-1", status: "delivered" },
